...
id_years <- read.csv('id_years_class_lower_v2.csv', row.names = 1)

save.image("id_years_class_lower_v2.RData")
...
for (var in c('depression', 'stroke_i', 'stroke_h', 'diabetes', 'hyperchol', 'hypertension', 'comorbidity',
              ...
              'quinolone', 'Ca_blocker_cardiac', 'urological', 'vasodilator')){
...
rm(dates, var, dis_other, morbidity, depression, stroke_i, stroke_h, hyperchol, hypertension, diabetes, dementia, apoe, early_years, dem_meds, comorbidity)
save.image("id_years_class_lower_cleaned_v2.RData")
...
# remove outliers
columns <- c('aa_duran_poly', 'ace_inh', 'aminoglycoside', 'antiarrhythmic', 'anticholinergic', 'antidepressant', 
             'antiepileptic', 'antigout', 'antihistamines', 'antiinflammatory', 'antimigraine', 
             'antipropulsive', 'antipsychotic', 'antithrombotic', 'anxiolytic', 'arteriolar', 
             'belladonna', 'penicillin', 'glucose_lowering', 'cardiac_glycoside', 
             'corticosteroid', 'anti_cough', 'dopaminergic', 'constipation', 
             ...
             'sedative', 'immunosuppressant', 'diuretic_low', 'macro_linco_strepto', 'muscle_relaxant', 
             'decongestant', 'opioid', 'antibacterial_other', 'OAD_systemic', 'K_sparing', 'propulsive', 
             'quinolone', 'Ca_blocker_cardiac', 'urological', 'vasodilator', 'deprivation', 'comorbidity')
for (col in columns){
  if (is.numeric(id_years[[col]])){
...
for (col in colnames(subset(id_years, select=c(ace_inh, aminoglycoside, antiarrhythmic, anticholinergic, antidepressant, 
                                               antiepileptic, antigout, antihistamines, antiinflammatory, antimigraine, 
                                               antipropulsive, antipsychotic, antithrombotic, anxiolytic, arteriolar, 
                                               belladonna, penicillin, glucose_lowering, cardiac_glycoside, 
                                               corticosteroid, anti_cough, dopaminergic, constipation, 
...
# complex models
id_years <- filter(id_years, !is.na(comorbidity) & !is.na(antidepressant) & !is.na(antiepileptic) & !is.na(antihistamines) & !is.na(antimigraine)&
               !is.na(antipropulsive) & !is.na(antipsychotic) & !is.na(anxiolytic) & !is.na(penicillin) & !is.na(glucose_lowering) &
               !is.na(corticosteroid) & !is.na(acid_reflux) & !is.na(diuretic_high) & !is.na(sedative) &
               !is.na(decongestant) & !is.na(opioid) & !is.na(propulsive) & !is.na(urological) & !is.na(vasodilator) & 
               !is.na(antithrombotic) & !is.na(cardiac_glycoside) &
               !is.na(immunosuppressant) & !is.na(Ca_blocker_cardiac))

m_all <- coxph(data=id_years, Surv(person_time, dementia) ~ scale(RankNorm(antidepressant)) + scale(RankNorm(antiepileptic)) + 
                 scale(RankNorm(antihistamines)) + scale(RankNorm(antimigraine)) + scale(RankNorm(antipropulsive)) +
                 scale(RankNorm(antipsychotic)) + scale(RankNorm(anxiolytic)) + scale(RankNorm(penicillin)) + scale(RankNorm(glucose_lowering)) +
                 scale(RankNorm(corticosteroid)) + scale(RankNorm(acid_reflux)) + scale(RankNorm(diuretic_high)) +
