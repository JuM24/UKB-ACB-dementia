...
# remove all prescription years before year 0
id_years <- filter(id_years, year >= year_0)
...
# for those with dementia, remove rows after dementia diagnosis
id_years$date <- as.Date(parse_date_time(id_years$year, "Y"))
id_years <- filter(id_years, is.na(date_dementia) | date <= date_dementia)
...
# get age at year 0 for each participant
age_0 <- filter(id_years, year == year_0) %>% group_by(id) %>% summarise(age_0 = mean(med_age))
id_years <- merge(id_years, age_0, by='id')

# remove unnecessary columns

# save
rm(age_0,dis_other,apoe,comorbidity,dates,dem_meds,dementia,depression,diabetes,early_years,hyperchol,hypertension,morbidity,stroke_h,stroke_i)
save.image("id_years_joint.RData")
...
id_years$status <- 'alive'
id_years$status[id_years$dead == '1'] <- 'dead'
id_years$status[id_years$dementia == 2] <- 'dementia' # if dementia diagnosis before death

# set starting time to 0 for each participant and set observation time for every "measurement" (i.e., for every prescription year)
id_years$start <- id_years$year - id_years$year_0
id_years$stop <- id_years$start + 1
id_years$obstime <- id_years$start
...
# create a separate polypharmacy variable for the scale (=total medication count - anticholinergics according to scale)
id_years$aa_duran_poly <- id_years$meds_count - id_years$aa_duran_n 

# centre variables
id_years$aa_duran_centred <- scale(id_years$aa_duran, center = TRUE, scale = TRUE)
id_years$aa_duran_poly_centred <- scale(id_years$aa_duran_poly, center = TRUE, scale = TRUE)
id_years$age_0_centred <- scale(as.numeric(id_years$age_0), center = TRUE, scale = TRUE)
id_years$year_centred <- scale(id_years$year, center = TRUE, scale = TRUE)
id_years$deprivation_centred <- scale(id_years$deprivation, center = TRUE, scale = TRUE)
id_years$comorbidity_centred <- scale(id_years$comorbidity, center = TRUE, scale = TRUE)

# keep only relevant variables
id_years <- subset(id_years, select=c(aa_duran_centred, obstime, age_0_centred, sex, id, person_time, dementia,
                                      aa_duran_poly_centred, data_provider, year_centred, bmi_class, alc_freq,
                                      activity, comorbidity_centred, depression, stroke, diabetes, education, deprivation_centred,
                                      hyperchol, hypertension, apoe_carrier, status))

# remove rows where predictor is NA
id_years <- id_years[complete.cases(id_years), ]

# create id-based data frame for Cox model part of the joint model
id_years <- arrange(id_years, obstime)
id_years_id <- id_years[!duplicated(id_years$id), ]
...
# supplementary data frame for analysis
id_years.idCR <- crLong(id_years_id, statusVar = "status",
                        censLevel = 'alive', nameStrata = "CR")

# longitudinal part
lmeFit.dem <- lme(aa_duran_centred ~ obstime + age_0_centred + I(age_0_centred^2) + sex, 
                  random = ~ obstime | id, data = id_years, na.action = 'na.exclude', control=c(msMaxIter=100, opt='optim'))
...
# joint part
jointFit11.dem <- jointModel(lmeFit.dem, coxFit4.dem,
                             timeVar = "obstime", method = "spline-PH-aGH", CompRisk = TRUE,
                             interFact = list(value = ~ CR, data = id_years.idCR))