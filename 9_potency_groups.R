...
id_years <- read.csv('id_years_aa_groups_v2.csv', sep = '|', row.names = 1)
save.image("id_years_aa_groups_v2.RData")
...
# identify those with diagnosis of (or prescriptions for) dementia before time point 0 (and within a year of year 0), and those with prior multiple sclerosis
...
rm(apoe, dis_other, dates, depression, diabetes, dem_meds, comorbidity, hyperchol, hypertension, morbidity, early_years, dementia, stroke_h, stroke_i)
save.image("id_years_aa_groups_cleaned_v2.RData")
...
# remove outliers
cols <- c("aa_0_count", "aa_05_count", "aa_1_count", "aa_2_count", "deprivation", "comorbidity")
for (col in cols){
